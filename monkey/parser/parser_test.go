...
func TestIdentifierExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("expr is not *ast.Identifier. got=%T", stmt)
	}
...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("expr is not *ast.IntegerLiteral. got=%T", stmt)
	}
	if ident.Value != 5 {
	...
	if ident.TokenLiteral() != "5" {
		t.Errorf("ident.TokenLiteral is not %s. got=%s", "5", ident.TokenLiteral())
	}
}
